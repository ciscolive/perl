...
our $modules = {
  ...
  GlobalLB => {
    ...
    Pool => {
      ...
      get_fallback_ipv4        => 'pool_names',
      get_fallback_ipv6        => 'pool_names',
      get_fallback_lb_method   => 'pool_names',
      get_member_order         => { pool_names => 1, members => 1 },
      get_member_ratio         => { pool_names => 1, members => 1 },
      get_member_statistics    => { pool_names => 1, members => 1 },
    ...
    VirtualServerV2 => {
      get_address            => 'virtual_servers',
      get_description        => 'virtual_servers',
      get_enabled_state      => 'virtual_servers',
      get_ltm_virtual_server => 'virtual_servers',
      get_monitor_rule_v2    => 'virtual_servers',
...
sub new {
  ...
  $self->{_client} =
    SOAP::Lite->proxy( $self->{proto} . '://' . $self->{server} . ':' . $self->{port} . '/iControl/iControlPortal.cgi' )
    ->deserializer( BigIP::iControlDeserializer->new() );
  $self->{_client}->transport->http_request->header(
    'Authorization' => 'Basic ' . MIME::Base64::encode("$self->{username}:$self->{password}") );
...
sub __process_statistics {
  ...
  foreach ( @{ @{ $statistics->{statistics} }[0]->{statistics} } ) {
    my $type = $_->{type};
    $stat_obj{stats}{$type} =
      Math::BigInt->new( "0x" . unpack( "H*", pack( "N2", $_->{value}{high}, $_->{value}{low} ) ) )->bstr;
  }
...
sub get_all_service_statuses {
  ...
  foreach
    my $service ( @{ $self->_request( module => 'System', interface => 'Services', method => 'get_all_service_statuses' ) } )
  {
...
sub __save_configuration {
  ...
  $self->_request(
    ...
    data      => { filename => $filename, save_flag => $flag }
  );
...
sub download_configuration {
  ...
  while (1) {
    $data = $self->_request(
      ...
      data      => { config_name => $config_name, chunk_size => $chunk, file_offset => $offset }
    );
...
sub download_file {
  ...
  while (1) {
    $data = $self->_request(
      ...
      data      => { file_name => $file_name, chunk_size => $chunk, file_offset => $offset }
    );
...
sub get_trunk_interfaces {
  my ( $self, $trunk ) = @_;
  return @{
    @{$_[0]->_request( module => 'Networking', interface => 'Trunk', method => 'get_interface', data => { trunks => [$trunk] } )
    }[0]
  };
}
...
sub get_trunk_media_speed {
  ...
  return
    @{ $_[0]
      ->_request( module => 'Networking', interface => 'Trunk', method => 'get_media_speed', data => { trunks => [$trunk] } ) }
    [0];
}
...
sub get_trunk_media_status {
  ...
  return
    @{ $_[0]
      ->_request( module => 'Networking', interface => 'Trunk', method => 'get_media_status', data => { trunks => [$trunk] } ) }
    [0];
}
...
sub get_self_ip_vlan {
  my ( $self, $ip ) = @_;
  return
    @{ $self->_request( module => 'Networking', interface => 'SelfIP', method => 'get_vlan', data => { self_ips => [$ip] } ) }
    [0];
}
...
sub get_ltm_pool_members {
  my ( $self, $pool ) = @_;
  my @members;
  foreach ( @{ @{ $self->__get_pool_members( $pool, 'LocalLB' ) }[0] } ) {
    push @members, ( $_->{address} . ':' . $_->{port} );
  }
  return @members;
}
...
sub get_gtm_pool_members {
  my ( $self, $pool ) = @_;
  my @members;
  foreach ( @{ @{ $self->__get_pool_members( $pool, 'GlobalLB' ) }[0] } ) {
    push @members, $_->{member}->{address} . ':' . $_->{member}->{port};
  }
  return @members;
}
...
sub get_db_variable {
  my ( $self, $var ) = @_;
  return
    @{ $self->_request( module => 'Management', interface => 'DBVariable', method => 'query', data => { variables => [$var] } )
    }[0]->{value};
}
...
sub get_subscription_list {
  my $self = shift;
  my @subs;
  foreach ( @{ $self->_request( module => 'Management', interface => 'EventSubscription', method => 'get_list' ) } ) {
    push @subs, $_;
  }
  return @subs;
}
...
sub create_subscription_list {
  ...
  $args{min_events_per_timeslice} =~ /^(-)?\d+$/
    or return 'Request error: missing or incorrect "min_events_per_timeslice" parameter';
  ...
  return $self->_request(
    ...
    interface => 'EventSubscription',
    method    => 'create',
    data      => { sub_detail_list => [$sub_detail_list] }
  );
}
