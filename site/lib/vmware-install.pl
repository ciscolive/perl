...
sub install_or_upgrade {
  ...
  if (vmware_product() eq 'vicli') {
    print wrap('This installer has successfully installed the vSphere SDK for Perl.'
               . "\n\n", 0);
  }
...
sub install_content_vicli_perl {
   ...
   # install modules in @install using CPAN from internet
   #------------------------------------------------------------------------
   if ( ! $install_rhel55_local ) {
...
sub install_content_vicli {
  ...
  db_add_answer('BUILD_NUMBER', "16453907");
...
sub main {
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '7.0.0') and
